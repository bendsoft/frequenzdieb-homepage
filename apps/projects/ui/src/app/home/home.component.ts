...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log('hi')
    this.eventService.getConcert('5e94cb5a63f0ff677e5a2691').subscribe((concert: Concert) => {
    ...
    $(window).on('mousemove', (e) => {
      ...
      $('.date').css(
        'transform',
        `translate(${-50 + this.mouse.x * 20}%, ${-50 + this.mouse.y * 20}%) rotateY(${
          this.mouse.x * 30
        }deg)`
      )
      $('.date .shadow').css(
        'transform',
        `translate(-50%, ${-this.mouse.y * 130}%) scale(${this.mouse.y + 1.2}) rotateZ(${
          this.mouse.x * 10
        }deg)`
      )
      $('.next-concert').css(
        ...
        `translate(${-50 + -this.mouse.x * 20}%, ${-50 + -this.mouse.y * 20}%) rotateY(${
          -this.mouse.x * 30
        }deg) rotateZ(-13deg) rotateX(${this.mouse.y * 20}deg)`
      )
